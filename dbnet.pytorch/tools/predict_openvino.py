...
class Pytorch_model:
    ...
    def export_onnx(self):
        img = torch.zeros((1, 3, 480, 720))#.cuda()  # image size(1,3,320,192) iDetection
        ...
        try:
            ...
            f = ("./onnx-xml/dbnet.onnx") # filename
    ...
    def onnx2ir(self):
        os.system('python3.6 /opt/intel/openvino/deployment_tools/model_optimizer/mo_onnx.py \
                        -m ./onnx-xml/dbnet.onnx \
                        -o ./onnx-xml/ \
                        --log_level ERROR \
                        --data_type FP32')
    # -----Onnx runtime
    def init_onnx_session(self):
        self.ort_session = ort.InferenceSession("./onnx-xml/dbnet.onnx")

    def predict_onnx(self, img_path: str, is_output_polygon=False, short_size: int = 1024):
        assert os.path.exists(img_path), 'file is not exists'
        img = cv2.imread(img_path, 1 if self.img_mode != 'GRAY' else 0)
        if self.img_mode == 'RGB':
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        batch = {'shape': [(h, w)]}
        start = time.time()
        preds = self.ort_session.run(None, {'images': tensor.cpu().numpy()})
        t = time.time() - start
        print("infer time: (ms) ", t*1000)
        print("preds:", preds)
        preds = preds[0]
    ...
    def init_ie(self):
        self.onnx2ir()
        self.ie = IECore()
        self.dbnet = self.ie.read_network(model="./onnx-xml/dbnet.onnx")

        self.input_blob = next(iter(self.dbnet.input_info))
        self.out_blob = next(iter(self.dbnet.outputs))
        self.exec_net = self.ie.load_network(network=self.dbnet, device_name="CPU")

        # Read and pre-process input images
        self.b, self.c, self.h, self.w = self.dbnet.input_info[self.input_blob].input_data.shape

    def predict_vino(self, img_path: str, is_output_polygon=False, short_size: int = 1024):
        assert os.path.exists(img_path), 'file is not exists'
        img = cv2.imread(img_path, 1 if self.img_mode != 'GRAY' else 0)
        if self.img_mode == 'RGB':
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        tensor = tensor.unsqueeze_(0)
        start = time.time()
        preds = self.exec_net.infer(inputs={self.input_blob: tensor})
        t = time.time() - start
        print("infer time: (ms) ", t*1000)
        print("preds", preds)
...
def init_args():
    ...
    parser.add_argument('--model_path', default='./onnx-xml/model_best.pth', type=str)
...
if __name__ == '__main__':
    ...
    # 初始化网络
    model = Pytorch_model(args.model_path, post_p_thre=args.thre, gpu_id=None, save_wts=args.save_wts)
    if(args.onnx):
        model.export_onnx()
    if(args.save_wts):
        exit(0)
